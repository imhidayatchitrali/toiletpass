...
export const AddPublicToilet = () => {
  ...
  const { dailyCount, canAdd, incrementCount } = usePublicToilets(user?.uid || null);
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <div className="p-6">
              ...
              <div className="bg-blue-50 p-4 rounded-lg mb-6 flex items-start">
                ...
                <p className="text-sm text-blue-700">Vous pouvez ajouter jusqu'à 3 toilettes publiques gratuites par jour. Assurez-vous que l'emplacement est correct et que les toilettes sont réellement accessibles au public.</p>
              </div>

              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Nom du lieu</label>
                  <input type="text" required className="w-full px-3 py-2 border rounded-md" value={name} onChange={(e) => setName(e.target.value)} placeholder="ex: Toilettes Jardin des Tuileries" />
                </div>
                  ...
                  {showSuggestions && addressSuggestions.length > 0 && (
                    <div className="absolute z-10 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-auto">
                      {addressSuggestions.map((result, index) => (
                        <button key={index} type="button" className="w-full text-left px-4 py-2 hover:bg-gray-100 focus:outline-none" onClick={() => selectAddress(result)}>
                          {result.display_name}
                        </button>
                      ))}
                    </div>
                ...
                <div>
                  ...
                  <textarea className="w-full px-3 py-2 border rounded-md" rows={3} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ajoutez des informations utiles sur l'accès..." />
                </div>
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-gray-500">Cliquez sur la carte pour définir l'emplacement exact</p>
                </div>
                ...
                <div className="flex justify-end space-x-4">
                  <button type="button" onClick={() => navigate("/")} className="px-4 py-2 text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">
                    Annuler
                  </button>
                  <button type="submit" disabled={loading || !selectedPosition || !canAdd} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 flex items-center">
                    {loading ? (
                      <>
